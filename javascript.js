...
googleFontsLink.href = `https://fonts.googleapis.com/css2?family=${options.join(
  "&family="
)}&display=swap`;
...
dropdown1.addEventListener("change", () => {
  if (dropdown2.value === dropdown1.value) {
    alert(
      " Background color cannot match Font color. Please change background color"
    );
...
function autofunction() {
  ...
  intervalID2 = setInterval(() => {
    ...
    telements.forEach(
      (telement) => (telement.style.fontFamily = fontc[fontindex])
    );
...
freset.addEventListener("click", () => {
  clearInterval(intervalID2);
  telements.forEach((telement) => {
    telement.style.fontFamily = defaultFont;
...
color.forEach((col) => {
  ...
  nopt.style.backgroundColor = col;
  if (nopt.style.backgroundColor === "black") {
    nopt.style.color = "white";
  }
...
it.addEventListener("click", () => {
  telements.forEach((telement) => {
    telement.style.fontStyle =
      telement.style.fontStyle === "italic" ? "normal" : "italic";
  });
});

bo.addEventListener("click", () => {
  telements.forEach((telement) => {
    telement.style.fontWeight =
      telement.style.fontWeight === "bold" ? "normal" : "bold";
  });
});
